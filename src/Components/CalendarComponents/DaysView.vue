        ...
        <div class="ds-week-view-pane" ref="pane" :style="dayHeight">
          <div class="ds-week">
            <div class="ds-hour-list">
              <div class="ds-hour" v-for="(hour, i) in hours" :class="hourClasses[ i ]">
  ...
  methods: {
    scrollToEvent() {
      if (this.scrollToFirst) {
        ...
        let first = this.calendar.iterateDays().reduce(last, (day, first) =>
          day.iterateEvents().reduce(
            first,
            (event, first) =>
              Math.min(
                first,
                event.start.hour * Constants.MINUTES_IN_HOUR +
                  event.start.minute
              ),
            event => !event.fullDay
          )
        ...
        if (last !== first) {
          let dayDelta =
            (first - this.scrollBuffer) / Constants.MINUTES_IN_HOUR;
